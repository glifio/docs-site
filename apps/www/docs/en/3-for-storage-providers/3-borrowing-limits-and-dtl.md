...
2. Liquidation value - the SP's “collateral value” (defined [here](/for-storage-providers/sp-liquidation-values))
...
* To borrow from GLIF, the **maximum DTL ratio ranges from 75% to 90%,** depending on whether the Storage Provider joins the [GLIF+ program](/glif-storage-providers-loyalty-rewards/storage-provider-loyalty-card).
  ...
  * Storage Providers who join the [GLIF+ program](/glif-storage-providers-loyalty-rewards/storage-provider-loyalty-card) as Gold Card holders can borrow up to a 90% DTL. Details of GLIF+ program are shown in the [Tiers and Limits table](/glif-storage-providers-loyalty-rewards/tiers-and-limits).
...
![Borrow limit: DTL >](en-image-280.webp)

> [!WARNING]
> This graph illustrates the situation for SPs who did not join the GLIF+ program. For those who joined the [GLIF+ program](/glif-storage-providers-loyalty-rewards/storage-provider-loyalty-card), please refer to the [tier and limit table](/glif-storage-providers-loyalty-rewards/tiers-and-limits) for details.
