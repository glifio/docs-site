...
Read [here](/for-storage-providers/manage-miner-actors-with-agents) for more details.

## How does the guarantor model work?

The [guarantor model](/for-storage-providers/guarantor-model) allows an SP to use a larger, more valuable Miner Actor as collateral to support borrowing for other Miner Actors. In other words, the miner used as collateral does not also have to be the miner used for sealing.
