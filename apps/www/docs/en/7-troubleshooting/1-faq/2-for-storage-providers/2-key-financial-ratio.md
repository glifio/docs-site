...
[Debt-to-Liquidation (DTL)](/for-storage-providers/borrowing-limits-and-dtl) ratio must be less than 75%. If the SP joined the [GLIF+ program](/glif-storage-providers-loyalty-rewards/storage-provider-loyalty-card) as a Gold Card member, the DTL increases to 90%. Read [here](/for-storage-providers/borrowing-limits-and-dtl) for more details.

## What is “liquidation value” and how is it calculated?

[Liquidation value](/for-storage-providers/sp-liquidation-values) is the estimated worth of an SP's miners if their sectors were terminated, accounting for penalties and other losses. It is computed as:
...
[DTL](/for-storage-providers/borrowing-limits-and-dtl) is computed as:
...
To determine how much you can borrow from GLIF, you need to understand the [Debt-to-Liquidation (DTL) ratio](/for-storage-providers/borrowing-limits-and-dtl). DTL ratio must be less than 75%. If the SP joined the [GLIF+ program](/glif-storage-providers-loyalty-rewards/storage-provider-loyalty-card) as a Gold Card member, the DTL increases to 90%. Read [here](/for-storage-providers/borrowing-limits-and-dtl) for more details.\
...
For detailed uses of borrowed FIL, read [here](/for-storage-providers/uses-of-borrowed-fil) for more details.
