...
Borrowed funds can be used for sealing, or for [withdrawing](/for-storage-providers/uses-of-borrowed-fil) out of the system entirely. In order to become eligible for borrowing, SPs must add collateral to GLIF. You can read more about the rules [here](/for-storage-providers/borrowing-limits-and-dtl).
...
GLIF uses smart contracts that enable SPs to maintain ownership over their miner actors while borrowing FIL. Read [here](/for-storage-providers/manage-miner-actors-with-agents) for more details.
...
SPs can withdraw FIL as long as their DTL ratios stays below 75%. If the SP joined the [GLIF+ program](/glif-storage-providers-loyalty-rewards/storage-provider-loyalty-card) as a Gold Card member, the DTL increases to 90%. Read [here](/for-storage-providers/uses-of-borrowed-fil) for more details.
