...
Borrowed funds can be used for sealing, or for [withdrawing](/docs/3-for-storage-providers/4-uses-of-borrowed-fil) out of the system entirely. In order to become eligible for borrowing, SPs must add collateral to GLIF. You can read more about the rules [here](/docs/3-for-storage-providers/3-borrowing-limits-and-dtl).
...
GLIF uses smart contracts that enable SPs to maintain ownership over their miner actors while borrowing FIL. Read [here](/docs/3-for-storage-providers/7-manage-miner-actors-with-agents) for more details.
...
SPs can withdraw FIL as long as their DTL ratios stays below 75%. If the SP joined the [GLIF+ program](/docs/5-glif+-storage-providers-loyalty-rewards/1-storage-provider-loyalty-card) as a Gold Card member, the DTL increases to 90%. Read [here](/docs/3-for-storage-providers/4-uses-of-borrowed-fil) for more details.
