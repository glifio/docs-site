...
[Debt-to-Liquidation (DTL)](/3-for-storage-providers/3-borrowing-limits-and-dtl) ratio must be less than 75%. If the SP joined the [GLIF+ program](/5-glif+-storage-providers-loyalty-rewards/1-storage-provider-loyalty-card) as a Gold Card member, the DTL increases to 90%. Read [here](/3-for-storage-providers/3-borrowing-limits-and-dtl) for more details.

## What is “liquidation value” and how is it calculated?

[Liquidation value](/3-for-storage-providers/2-sp-liquidation-values) is the estimated worth of an SP's miners if their sectors were terminated, accounting for penalties and other losses. It is computed as:
...
[DTL](/3-for-storage-providers/3-borrowing-limits-and-dtl) is computed as:
...
To determine how much you can borrow from GLIF, you need to understand the [Debt-to-Liquidation (DTL) ratio](/3-for-storage-providers/3-borrowing-limits-and-dtl). DTL ratio must be less than 75%. If the SP joined the [GLIF+ program](/5-glif+-storage-providers-loyalty-rewards/1-storage-provider-loyalty-card) as a Gold Card member, the DTL increases to 90%. Read [here](/3-for-storage-providers/3-borrowing-limits-and-dtl) for more details.\
...
For detailed uses of borrowed FIL, read [here](/3-for-storage-providers/4-uses-of-borrowed-fil) for more details.
