...
When an SP borrows FIL for storage mining, it can borrow more FIL from GLIF because the borrowed FIL is included in the SP's [liquidation value](/docs/for-storage-providers/sp-liquidation-values). Therefore, the SP is less restricted by the [debt-to-liquidation ratio (DTL)](/docs/for-storage-providers/borrowing-limits-and-dtl) because the SP's liquidation value grows simultaneously with its debt. For example, imagine the following scenario:
...
> [!IMPORTANT]
> Regardless of if an SP chooses to use borrowed FIL for storage mining or withdrawals outside of GLIF, the SP's DTL ratio must stay below 75% - 90% (Depends on whether the SP joined the [GLIF+ program](/docs/glif-storage-providers-loyalty-rewards/storage-provider-loyalty-card)) in order to be approved.
